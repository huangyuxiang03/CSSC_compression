#include "ByteArrayOutputStreamArray.h"


template <typename T>
array<std::uint8_t> ToByte(T input)
{
	std::uint8_t* bytePointer = reinterpret_cast<std::uint8_t*>(&input); 
	return array<std::uint8_t>(bytePointer, bytePointer + sizeof(T));
}
template <typename T>
array<std::uint8_t>& operator>>(array<std::uint8_t>& in, T& out)
{
	if (in.size() >= sizeof(T)) {
...
void ByteArrayOutputStream::write(int b){
   array<std::uint8_t> bytes_tmp = ToByte(b);
...
void ByteArrayOutputStream::writeBytes(char b[]){
	int len = sizeof(b);
	for (int i=0; i < len;i++) {
		array<std::uint8_t> byte_tmp = ToByte(b[len - i - 1]);
        this->bytes.insert(this->bytes.end(), byte_tmp.begin(), byte_tmp.end());
		array <std::uint8_t>().swap(byte_tmp);
    }
	//delete[] b;
}
...
void ByteArrayOutputStream::readFromFile()
{
	...
	else
	{
		...
		for (int col_i_i = 0; col_i_i < col_n; col_i_i++) {
			array<std::uint8_t> bytes_col_pos;
...
array<std::uint8_t> ByteArrayOutputStream::getInt()
{
...
array<std::uint8_t> ByteArrayOutputStream::getBytes()
{
	return bytes;
}
...
array<std::uint8_t> ByteArrayOutputStream::getColBytes()
{
   array<std::uint8_t> getbytes;
   ...
   //array<std::uint8_t> newgetbytes(decompdstlength);
...
array<std::uint8_t> ByteArrayOutputStream::getColBytesGZip()
{
	array<std::uint8_t> getbytes;
	getbytes.insert(getbytes.begin(), bytes.begin(), bytes.begin() + col_pos[col_index]);
	bytes.erase(bytes.begin(), bytes.begin() + col_pos[col_index]);
...
// get bit_array compressed part
array<std::uint8_t> ByteArrayOutputStream::getBytesLength(int length)
{
	array<std::uint8_t> getbytes;
...
void ByteArrayOutputStream::writeRowCol(int col_nf, int* col_posf)
{
	...
	else
	{
		...
		for (int i = 1; i < col_nf + 1; i++) {
			array<std::uint8_t> bytes_col_pos = ToByte(col_posf[i]);
			//cout << "col_posf[i] :" << col_posf[i] <<endl;
			for (std::uint8_t bt : bytes_col_pos) outfile << bt;
		}
		array<std::uint8_t> bytes_col_n = ToByte(col_nf);
