...
void IntPacker::pack8Values(int* values, int offset, std::uint8_t* buf)
{
    int bufIdx = 0;
    int valueIdx = offset;
...
void IntPacker::unpack8Values(std::uint8_t* buf, int offset, int* values)
{
    int byteIdx = offset;
    long buffer = 0;
...
void IntPacker::unpackAllValues(std::uint8_t* buf, int length, int* values)
{
    int idx = 0;
    int k = 0;
    while (idx < length) {
