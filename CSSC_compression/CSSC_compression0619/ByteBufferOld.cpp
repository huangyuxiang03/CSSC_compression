...
int ByteBufferOld::remaining()
{
	return bytes.size();
}

bool ByteBufferOld::hasRemaining()
{
	if (bytes.size() > 0) return true;
	return false;
}

int ByteBufferOld::readInt()
{
	int buffer_int = 0;
...
std::uint8_t ByteBufferOld::readuchar()
{
	std::uint8_t uc;
	this->bytes << uc;
	return uc;
...
long long ByteBufferOld::readLong()
{
...
void ByteBufferOld::get(char* dst, int len)
{
	int max = len;
...
void ByteBufferOld::get(std::vector<uint8_t>& tmp, int offset, int length)
{
	if (length > remaining()) {
...
void ByteBufferOld::divideTo3Parts(ByteBufferOld& b2, ByteBufferOld& b3) {
	int length_b3;
